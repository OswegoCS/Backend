...
import com.oswego.pcr.repositories.mappers.UserRowMapper;
...
public class UserDaoImpl implements UserDao {
     ...
     */
    private String insertUsers = "INSERT INTO users (first_name, last_name, email, screen_name, course) VALUES ";
    ...
    public User findByEmail(String email) {
        try {
            ...
            List<Role> roles = template.query(SELECT_ROLES, new MapSqlParameterSource("id", user.getId()),
    ...
    public List<User> findByCourse(String course) {
        try {
            List<User> students = template.query(SELECT_BY_COURSE, new MapSqlParameterSource("course", course),
                    userMapper);
            return students;
        } catch (EmptyResultDataAccessException e) {
    ...
    public User findCourseInstructor(String course) {
        try {
            User instructor = template.queryForObject(SELECT_INSTRUCTOR_BY_COURSE_ID,
                    new MapSqlParameterSource("id", course), BeanPropertyRowMapper.newInstance(User.class));
            return instructor;
        } catch (Exception e) {
            // Throw an exception, if we are not getting an instructor back, there are
            // problems
            throw new ResponseStatusException(HttpStatus.INTERNAL_SERVER_ERROR, e.getMessage());
        }
    }
